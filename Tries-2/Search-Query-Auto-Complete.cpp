...
class TrieNode{
    public:
        unordered_map<char, TrieNode*> children; //made map instead of array to store characters beyond 'a-z'
...
public:
    ...
    static bool cmp(pair<string, int>&a, pair<string, int>&b){
        if(a.second > b.second) return true;    //priority to frequency
    ...
    vector<string> input(char c) {
        if(c=='#'){
            insert(inSen, 1);   //insert current sentence
            pnode = root;
            inSen = "";
            return {};
        }
